...
> **样式优先级（最高）**: 「强制规范」章节的代码样式规则优先级高于一切其他考虑——功能正确性之外，任何理由（简洁、Go 社区惯例、个人偏好、参考代码的风格）都不能作为偏离样式的借口。**每次写完/改完代码，交付前必须逐条自查「函数要求」清单**；发现既有代码样式不合规时，触碰该文件即顺带修复，不合规样式不得在新代码中出现。
...
- 函数体与大逻辑块的开括号 `{` 之后必须空一行，再写块内内容；大逻辑块之间用空行分隔；**此规则仅适用于非匿名函数（普通函数/方法声明）**，匿名函数（`defer func() {...}()`、`go func() {...}()`、作为参数的闭包回调如 `func(o *chatOptions) {`）开括号后**不空行**，直接写内容
