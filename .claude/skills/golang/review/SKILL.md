...
对 Go 代码变更做业务逻辑与风险审查：确认实现是否符合意图，排查 Bug、遗漏、回归，以及安全、并发、死锁、泄漏、性能问题。代码格式与风格不属于本技能职责（由 coding 承载）。

## 审查流程

1. **理解变更意图**：先明确本次改动要实现什么功能、解决什么问题（依据需求、issue、提交说明、注释、函数名），再对照代码判断实现是否符合预期
2. **识别变更范围**：执行 `git diff -- '*.go'` 聚焦修改的文件，并找出其调用方
3. **测试覆盖检查**：本次修改/新增的代码若无对应单元测试，先调用 testing 按其规范补充，再继续审查
4. **静态分析辅助**（运行 go 命令前先按 where 定位 go 版本）：`go vet ./...`，测试按 testing「测试命令」只运行本次修改相关包的测试（含 `-race`），不全量 `go test ./...`（全量仅提交前回归使用）；静态检查通过不等于业务正确
5. **逐维度审查**：按下文维度逐项检查，问题按严重程度归类
6. **修复后验证**：修复 bug 先按 testing 写失败测试复现；修复后重新运行测试与静态分析，确认无回归
...
## 审查范围
- 修改文件：src/core/user/user.go
- 影响调用方：src/api/handler/user.go

## 发现问题
[严重] 数据竞争
- 位置：src/core/cache/cache.go:20
- 问题：共享 map 无同步访问, 并发读写会 panic 或数据错乱
- 修复：加 sync.RWMutex 保护

[高] 业务逻辑遗漏
- 位置：src/core/order/order.go:45
- 问题：用户不存在时返回空结果, 调用方无法区分"无数据"与"出错"
- 修复：返回哨兵错误 ErrUserNotFound

## 摘要
- 严重：1  高：1  中：0
- 建议：阻塞合并, 修复后重新运行相关包测试与 go vet ./...
```
