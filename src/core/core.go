...
func init() {
	...
	// 解析 --conf 命令行参数，允许外部显式指定配置文件路径
	flag.StringVar(&configFile, "conf", "", "config file path")
	flag.Parse()

	// 未通过命令行指定时，依次回退：CFG_CONFIG 环境变量 -> CFG_PATH 环境变量 -> 默认路径
	if configFile == "" {
	...
	// 本地环境且未配置任何 MySQL DSN 时，降级为 SQLite mock，便于本地开发调试
	if Conf.IsLocal() && Conf.MySQL.Reader == "" && Conf.MySQL.Writer == "" {
		...
		logger.Printf("MySQL: reader=%s writer=%s", Conf.MySQL.Reader, Conf.MySQL.Writer)
	}

	// 本地环境且未配置 Redis 地址时，降级为 miniredis mock
